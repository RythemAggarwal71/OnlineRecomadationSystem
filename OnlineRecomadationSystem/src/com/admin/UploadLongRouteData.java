...
/**
 * Servlet implementation class UploadLongRouteData
 */
@WebServlet("/UploadLongRouteData")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 50, // 50MB
maxFileSize = 1024 * 2048 * 100, // 100 MB

maxRequestSize = 1024 * 2048 * 1000)
public class UploadLongRouteData extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
			tablename="longbusroute";
		}
		else if(transporttype.equals("Train"))
		{
			tablename="longtrainroute";	
		}
		else
		{
			tablename="longplainroute";
		}
		...
		if (filePart != null) {
			...
			if (filePart.getContentType().startsWith("image/")
				...
				while ((line = br.readLine()) != null) {
					...
					if(i>1)
					{
							try {
								Connection con;
								con = Dbconn.conn();
								String sql = "insert into "+tablename+"(src,dest,points,routeno,distance,time,price,bordingpoint) values(?,?,?,?,?,?,?,?)";
				...
				RequestDispatcher rd = request.getRequestDispatcher("/UploadLongRoute.jsp");
