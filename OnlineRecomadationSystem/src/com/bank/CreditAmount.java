...
/**
 * Servlet implementation class CreditAmount
 */
 		...
 		ResultSet rs=st1.executeQuery("select * from bankinfo where User='"+uname+"' and BankName='"+bank+"'");
 				if(rs.next())
 				{
 			        ...
 			        if(rs2!=0)
 			        {
 	 					...
 	 					st.executeUpdate("insert into transaction values('"+uname+"','"+bank+"','"+Amount1+"',00.0,'"+crntbal1+"','"+ts+"')");
 
 	 					pw.println("<script> alert(' Credited Successfully');</script>");	
 	 					RequestDispatcher rd=request.getRequestDispatcher("CreditAmount.jsp");
 	 					rd.include(request, response);
 			        }
